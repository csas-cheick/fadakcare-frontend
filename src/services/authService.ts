...
const refreshToken = async (): Promise<boolean> => {
  ...
  try {
    const response = await fetch('http://localhost:5120/api/auth/refresh', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const logout = async () => {
  ...
  if (refreshTokenValue && userId) {
    try {
      await fetch('http://localhost:5120/api/auth/logout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: Number(userId), refreshToken: refreshTokenValue })
      });
...
const forgotPassword = async (
  ...
  const response = await fetch("http://localhost:5120/api/auth/forgotPassword", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
const verifyCode = async (
  ...
  const response = await fetch("http://localhost:5120/api/auth/verifyCode", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
const resetPassword = async (data: ResetPasswordRequest): Promise<ResetPasswordResponse> => {
  try {
    const response = await fetch("http://localhost:5120/api/auth/resetPassword", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
