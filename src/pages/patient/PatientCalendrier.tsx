...
const PatientCalendrier: React.FC = () => {
  ...
  const loadRendezVous = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const rendezVousList = await RendezVousService.getPatientRendezVous(patientId);
      const calendarEvents = RendezVousService.convertToCalendarEvents(rendezVousList);
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      await RendezVousService.deleteRendezVous(parseInt(selectedEvent.id));
  ...
  return (
    <>
      <PageMeta
        title="Mon Calendrier | FadakCare - Plateforme Médicale"
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Visualisez vos rendez-vous médicaux
              </p>
            </div>
            {loading && (
        ...
        {selectedEvent && (
          <div className="flex flex-col">
            ...
            <div className="space-y-4">
              ...
              {selectedEvent.extendedProps.medecinId && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1">
                    Médecin assigné
                  </label>
                  <p className="text-gray-900 dark:text-white">
                    Dr. {selectedEvent.extendedProps.medecinId}
                  </p>
                </div>
              )}
            </div>
...
export default PatientCalendrier;
