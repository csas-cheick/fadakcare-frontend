...
const PatientAlertesPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [alertesRes, profilRes] = await Promise.all([
        HttpService.get<Alerte[]>(`http://localhost:5120/api/alerte/${userRole}/${userId}`),
  HttpService.get<{ medecinId?: number }>(`http://localhost:5120/api/compte/profil/patient/${userId}`)
      ]);
  ...
  const envoyerAlerte = async () => {
    ...
    try {
      const res = await HttpService.post(`http://localhost:5120/api/alerte/envoyer`, {
                    ...
                    >
                      <div className="flex items-start gap-4">
                        <div className="relative">
                          <div className={`w-12 h-12 ${getAvatarColor(alerte.expediteurRole)} rounded-full flex items-center justify-center text-white font-semibold`}> 
                            {alerte.expediteurRole === 'patient' ? 
                              getInitials(alerte.destinataireNom) : 
                              getInitials(alerte.expediteurNom)}
                          </div>
